...
lineaccesstoken = '3PSwl1LBiTM9S5ud4j9qx1NeOXYlm26jMeWuLBcfd+9FPD24UF1nzNgQny5Aa8kLrp5mZngWFzft26Lz36gUN2aOgmNL2vIovMn5BbrES77fYgtGMCX6uYF4zfkRj4OLqdw1I1g/ACOMz56TNe4FpQdB04t89/1O/w1cDnyilFU='
...
def event_handle(event):
    ...
    if msgType == "text":
        ...
        if msg == "สวัสดี" :
            replyObj = TextSendMessage(text="YEET")
            line_bot_api.reply_message(rtoken, replyObj)
        elif msg == "กินข้าวมั้ย" :
            replyObj = TextSendMessage(text="เป็นบอทกินไม่ได้")
            line_bot_api.reply_message(rtoken, replyObj)
        elif msg == "เล่นเกมมั้ย" :
            replyObj = TextSendMessage(text="ไม่เล่น")
            line_bot_api.reply_message(rtoken, replyObj)
        else :
